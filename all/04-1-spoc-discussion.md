...
1. 寄存器、高速缓存、内存、外存的访问特征？
   * 访问时间：寄存器1ns，高速缓存2ns，内存10ns，外存磁盘10ms，磁带100s
   * 存储空间由小到大：寄存器，高速缓存，内存，外存
2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？
   * 理想存储器的访问速度，存储容量，价格不可兼得，因此形成层次结构
   * 层次结构中最佳状态是所有存储介质都在工作，又均衡，即没有哪一层成为瓶颈，所以叫均衡繁忙
3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？
   * 遇到过，当时我优化程序的结构，减少内存使用

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
   * 覆盖技术：把内存分为若干块，不会同时执行的模块可共享同一个内存块
   * 主要工作：将程序分为模块，并确定哪些模块可以共享同一块内存
2. 什么是交换技术？覆盖与交换有什么不同？
   * 以进程为单位，交换时将整个进程的数据换入换出
   * 不同：覆盖是解决一个程序占用空间大内存空间，交换是解决多个进程在内存中，占用空间大于内存空间
...
1. 什么是时间局部性、空间局部性和分支局部性？
   * 时间局部性：程序访问内存后，很可能在短时间内再次访问这个地址
   * 空间局部性：程序访问内存后，很可能会访问这个地址相邻的地址
   * 分支局部性：如果在空间-时间坐标空间中，路径的预期部分只有几个可能的替代方案
2. 如何提高程序执行时的局部性特征？
...
### 8.5 虚拟页式存储
 ...
 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？
    * 只将当前程序需要的部分页面加载进内存中
    * 功能：将相应缺失的页面加载进内存中，恢复程序的正常执行
 1. 为了支持虚拟页式存储的实现，页表项有什么修改？
    * 多了一些标志位
      * 保留位
      * 缓存有效位
      * Write-though/write-back位
...
1. 缺页异常的处理流程？
   * 访问页表项，得知页面是无效的，通知操作系统进入缺页异常流程
   * 首先找到外存中的该页面
   * 寻找内存中是否有空闲的页面，如果有，将该页存入内存中的空闲页面
   * 如果找不到空闲页面，根据页面替换算法找一个内存中不常用的页替换出去
   * 修改页表项（除该页外，如果是替换出去的还要修改被替换的页表项）


2. 虚拟页式存储管理中有效存储访问时间是如何计算的？
   * 缺页率 p 时，$$访问时间 = 访存时间\times(1 - p)+缺页异常处理时间\times p$$
...
把i，j对调就可以看出差别了。```A[i][j]``` 是局部性好的，```A[j][i]```是局部性差的
